...
export const LearningSideBarContent = ({
  currentComponent = 'Controller',
  nodeInfo,
}: LearningSideBarContentProps) => {
  ...
  const diagramRelations = t(`components.${componentKey}.diagramRelations`, {
    returnObjects: true,
  }) as string[];
    ...
    >
      ...
      >
        ...
        <Typography variant="body1" sx={{ mb: 2 }}>
          {t(`components.${componentKey}.description`)}
        </Typography>
        <ul>
          {Array.isArray(diagramRelations) &&
            diagramRelations.map((item: string, idx: number) => (
              <li key={idx}>{item}</li>
            ))}
        </ul>
      </Box>
