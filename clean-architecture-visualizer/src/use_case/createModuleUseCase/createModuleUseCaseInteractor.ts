...
export class CreateModuleUseCaseInteractor implements CreateModuleUseCaseInputBoundary {
    ...
    async execute(inputData : CreateModuleUseCaseInputData) {
        try {
            ...
            const featuresDirectory = await this.fileAccess.bfsFindDir(currPath, 'features');
            if (!featuresDirectory) {
                this.presenter.showFailView("The features directory does not exist. Please initialize the project first.");
                return;
            }

            // check if feature exists
            const currFeatureDirectory = await this.fileAccess.bfsFindDir(featuresDirectory as string, feature);
            if (!currFeatureDirectory) {
                this.presenter.showFailView("The input feature does not exist in the features directory.");
                return;
            }
            ...
            const ucFiles = ['InputBoundary','InputData','Interactor','OutputData','OutputBoundary'];
